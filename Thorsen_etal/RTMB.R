...
# Generate and plot INLA mesh for location of Norwegian municipalities
...
x = tau^(-2)*chol(COV)%*%rnorm(n_s)      # Simulate latent Gaussian field
dat = NULL
...
dat$y = rpois(n,lambda=exp(beta0+beta1*dat$rainfall+x[dat$meshidxloc]))   # Poisson response
...
# R version of the C++ objective function
f <- function(parms) {
  ...
  eta = beta0 + beta1*rainfall + x[meshidxloc]/tau
  for(i in 1:n)
    nll = nll - log_dzipois(y[i], exp(eta[i]), p)
...
# Fit the model using true paramer values as starting values
par_init <- list(beta0=beta0, beta1=beta1, log_tau=log(tau), log_kappa=log(kappa), p=p, x=rep(0.0,n_s))
obj <- MakeADFun(f, par_init, random="x")   # Build TMB model from R code. Argument "random" invokes the Laplace approximation for x
